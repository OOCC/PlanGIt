...
#define LOG_TIME_WARN_FMT                    "[%s]\033[1;33m[WARN]\033[0m    "  //warn log info fmt (color: yellow)
#define LOG_TIME_ERROR_FMT                   "[%s]\033[0;31m[ERROR]\033[0m   "  //error log info fmt (color: red)
...
{ \
	...
	\
	printf(LOG_TIME_INFO_FMT, timeStr); \
...
{ \
    ...
    strftime(timeStr,  timeStrLen, "%Y-%m-%d %H:%M:%S", localtime(&curTime));  \
	\
	printf(LOG_TIME_WARN_FMT, timeStr); \
    PRINT_LOG_BASE(format , ##args); \
    printf(LOG_FILE_LINE_FMT, __FILE__, __LINE__); \
    printf("\n"); \
}
...
{ \
    ...
    strftime(timeStr,  timeStrLen, "%Y-%m-%d %H:%M:%S", localtime(&curTime));  \
	\
	printf(LOG_TIME_ERROR_FMT, timeStr); \
    PRINT_LOG_BASE(format , ##args); \
    printf(LOG_FILE_LINE_FMT, __FILE__, __LINE__); \
    printf("\n"); \
}
